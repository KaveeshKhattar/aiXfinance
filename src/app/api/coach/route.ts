...
export async function POST(req:Request){try{
 ...
 if(body.useAgent){if(!elevenConfigured()){warning='ElevenLabs key not configured. Using local playbook.';coachSource='local_fallback';}else{try{const result=await runAgent(body.sessionId||crypto.randomUUID(),'coach',body.turns,account,broker,body.meetingSlots);events=result.events;coachSource='elevenlabs_agent';
